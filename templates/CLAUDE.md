...
- Tool settings live in each tool's own config file (`ruff.toml`, `pyrefly.toml`, `pytest.toml`, `tach.toml`, `noprim.toml`), never in `pyproject.toml`.
...
- **Never allow primitives as function parameters.** Wrap them in a `NewType` or a pydantic `RootModel` so each value carries its domain meaning — e.g. `UserId = NewType("UserId", int)` rather than a bare `int`. `noprim` enforces this. Pre-existing violations are recorded in `noprim-baseline.json`; remove an entry when you wrap the value rather than adding new ones.
