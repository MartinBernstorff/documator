...
ERROR   0 files, 0 warnings, 1 error
ERROR   @Foo.md: "Foo" is not a valid skill name
""")
...
def test_dry_run_is_another_spelling_of_check(tmp_path: Path) -> None:
    build_tree(
        tmp_path,
        TreeLayout("""
            in
    ...
    args = ["render", str(tmp_path / "in"), str(tmp_path / "out"), "--dry-run"]
    assert main(args) == 1

    assert_tree(tmp_path / "out", TreeLayout(""))


def test_render_dry_run_combined_with_watch_exits_two(tmp_path: Path) -> None:
    build_tree(tmp_path, TreeLayout("in\nout"))
...
def test_render_adopt_flag_takes_over_a_foreign_file(tmp_path: Path) -> None:
    build_tree(
        tmp_path,
        TreeLayout("""
            in
    ...
    args = ["render", str(tmp_path / "in"), str(tmp_path / "out"), "--adopt"]
    assert main(args) == 0
...
def test_render_adopt_combined_with_watch_exits_two(tmp_path: Path) -> None:
    build_tree(tmp_path, TreeLayout("in\nout"))

    args = ["render", str(tmp_path / "in"), str(tmp_path / "out"), "--adopt", "--watch"]
    assert main(args) == 2


def test_skills_adopt_flag_takes_over_a_foreign_file(tmp_path: Path) -> None:
    build_tree(
        tmp_path,
        TreeLayout("""
            in
    ...
    args = ["skills", str(tmp_path / "in"), str(tmp_path / "out"), "--adopt"]
    assert main(args) == 0

    assert "# Foo" in (tmp_path / "out" / "foo" / "SKILL.md").read_text()


def test_skills_check_flag_writes_nothing(tmp_path: Path) -> None:
    build_tree(
        tmp_path,
        TreeLayout("""
            in
    ...
    args = ["skills", str(tmp_path / "in"), str(tmp_path / "out"), "--check"]
    assert main(args) == 1

    assert_tree(tmp_path / "out", TreeLayout(""))


def test_skills_check_combined_with_watch_exits_two(tmp_path: Path) -> None:
    build_tree(tmp_path, TreeLayout("in\nout"))

    args = ["skills", str(tmp_path / "in"), str(tmp_path / "out"), "--check", "--watch"]
    assert main(args) == 2


def test_adopt_survives_quiet_with_a_count(tmp_path: Path) -> None:
    build_tree(
        tmp_path,
        TreeLayout("""
            in
...
WARNING adopted 1 file not previously written by documator
""")
